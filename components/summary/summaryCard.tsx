...
const StatusBagde = ({status}: {status: string}) => {
    return <span className={cn(`px-3 py-2 text-xs font-medium rounded-full capitalize ` , status === 'completed' ? 'bg-green-300 text-green-800': 'bg-yellow-300 text-yellow-800 ')}>{status}</span>
}
